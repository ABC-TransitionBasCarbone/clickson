...
export const ActivityDataForm = ({ dataToFill }: ActivityDataFormProps) => {
  ...
  const getCategoryData = async () => {
    ...
    setSessionSubCategories(
      sessionCategory.sessionEmissionSubCategories.map((subcategory) => ({
        ...subcategory,
        ...
        <Stack key={category.id}>
          ...
          <Stack spacing={2} marginTop={2} marginBottom={2} sx={{ flexDirection: 'row' }}>
            ...
            <Stack sx={{ marginLeft: '24px !important', flex: 1 }}>
              <QuestionTypeComponent schoolYear={school?.establishmentYear} sessionSubCategoryProp={category} />
            </Stack>
          </Stack>
        </Stack>
      ))}
